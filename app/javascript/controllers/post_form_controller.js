...
export default class extends Controller {
  ...
  async fetchTitle(e) {
    ...
    let resp = await post(this.scrapUrlValue, {
      query: { url: this.url },
      responseKind: "json",
    });
  ...
  _initTags() {
    ...
    this.tagify.on("add", () => {
      this._updateTagsInput();
    });
    this.tagify.on("remove", () => {
      this._updateTagsInput();
    });

    let fetchTags = debounce((e) => {
      this._fetchTags(e.detail.value, true);
    }, 300);
  ...
  async _fetchTags(value, showDropdown = false) {
    ...
    if (resp.ok) {
      let data = await resp.json;
      this.tagify.whitelist = data.map((tag) => {
        return tag.name;
      });
